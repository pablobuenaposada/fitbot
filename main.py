...
for _class in classes:
    for goal in settings.BOOKING_GOALS:
        ...
        if goal_time is not None and goal_time in _class['timeid']:  # if this class is a match for this user book it
            response = session.post(constants.BOOK_ENDPOINT, data={'id': _class['id'], 'day': target_day_str, 'insist': 0, 'familiId': ''})
            if response.status_code == 200 and response.json()['bookState']:
                print(f'Session booked for {settings.EMAIL} on {target_day_str} at {_class["timeid"]}')
            else:
